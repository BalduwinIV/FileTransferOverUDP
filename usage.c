...
void print_usage() {
    ...
    printf("\t--file=FILENAME\t\t\t\tDefines the name of file, that yo want to send.\n\n");
    printf("commands:\n");
    printf("\tdata_sender --ip=LOCAL_IP --port=LOCAL_PORT listen\n\t\t\t\t\t\tStart listening.\n");
    printf("\tdata_sender stop\n\t\t\t\t\t\tStop listening.\n");
    printf("\tdata_sender --ip=LOCAL_IP --port=LOCAL_PORT --dest_ip=TARGET_IP --dest_port=TARGET_PORT --file=FILENAME send\n\t\t\t\t\t\tSend file \"FILENAME\" to TARGET_IP:PORT address.\n");
}
