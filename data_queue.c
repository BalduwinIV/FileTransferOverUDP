...
void add_file(DATA_queue_t *data_queue, char *filename, unsigned char *file_hash) {
    ...
    memcpy(data_queue->files[data_queue->files_n].file_hash, file_hash, SHA256_DIGEST_LENGTH);
...
void remove_file(DATA_queue_t *data_queue, unsigned char *file_hash) {
    for (uint32_t file_i = 0; file_i < data_queue->capacity; file_i++) {
        if (data_queue->files[file_i].file_hash == 0) {
            continue;
        } else if (strcmp((char *)data_queue->files[file_i].file_hash, (char *)file_hash) == 0) {
...
DATA_file_t * find_file_by_hash(DATA_queue_t *data_queue, unsigned char *file_hash) {
    for (uint32_t file_i = 0; file_i < data_queue->capacity; file_i++) {
        if (strcmp((char *)data_queue->files[file_i].file_hash, (char *)file_hash) == 0) {
